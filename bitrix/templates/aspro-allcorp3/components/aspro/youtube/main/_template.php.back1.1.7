			...
			<?endif;?>
				<div class="carousel-youtube">
					<div class="item-views-youtube front blocks">
						<div class="owl-carousel <?=$owlClasses?>" data-plugin-options='{"items": <?=$arParams['COUNT_VIDEO_ON_LINE_YOUTUBE']?>, "nav": true, "autoplay": false, "rewind": true, "marginMove": true, "margin": <?=($bItemsOffset ? '32' : $margin)?>, "loop": true, "dots": true, "dotsContainer": false, "responsive" : {"0": {<?=($arParams['TEXT_CENTER'] ? '' : '"autoWidth": false, "lightDrag": false,')?> "items": <?=$items0?>, "dots": <?=$bDots0?>, "margin": <?=($bItemsOffset ? '24' : '0')?>}, "380": {<?=($arParams['TEXT_CENTER'] ? '' : '"autoWidth": false, "lightDrag": false,')?> "items": <?=$items380?>, "dots": 1, "margin": <?=($bItemsOffset ? '24' : '0')?>}, "768": {"autoWidth": false, "lightDrag": false, "dots": <?=$bDots768?>, "items": <?=$items768?> }, "1200": {"items": <?=$items1200?>, "dots": <?=$bDots1200?>}}}'>
							<?foreach ($arResult['ITEMS'] as $item):?>
								<div class="video_wrapper">
									<div class="youtube-list__item">
										<iframe width="<?=$iframeWidth;?>" height="<?=$iframeHeight;?>" src="https://www.youtube.com/embed/<?=$item['ID']?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
