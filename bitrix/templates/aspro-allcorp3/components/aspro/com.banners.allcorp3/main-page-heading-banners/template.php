	...
	<div class="banners-big front<?=($bHideOnNarrow ? ' hidden_narrow' : '')?> <?=$bannerClasses?>">
		<div class="maxwidth-banner <?=$arParams['NARROW_BANNER'] ? 'maxwidth-theme' : ''?>">
			<div class="banners-big__wrapper">
				<div class="swiper-container main-slider <?=$carouselClasses?>"  data-plugin-options='<?=json_encode($arOptions)?>'>
						...
						<?/*<div class="banners-big__item-wrapper">*/?>
							...
							<?if ($bShowVideo) {?>
								<div class="wrapper_video">
									<video class="video" autoplay="" autobuffer="" playsinline="" webkit-playsinline="" id="video_banner" class="video" loop="" muted="">
							...
							<?}?>
								...
								<div class="<?=($bHasUrl ? ' wurl' : '')?> banners-big__item  banners-big__depend-height <?=(!$arParams['HEADER_OPACITY'] ? '' : 'banners-big__depend-padding');?> <?=$bannerItemClasses?> <?=$bOnlyImage && $bShowVideo ? 'banners-big__item--img-with-video' : '';?>">
									...
									<?endif;?>
									<div id="<?=$this->GetEditAreaId($arItem['ID'])?>" class="<?=$arParams['NO_MAXWITH_THEME_WIDE'] ? '' : 'maxwidth-theme'?> pos-static <?=($bOnlyImage && $bLinkOnName ? ' fulla' : '')?> <?=($bVideoAutoStart ? 'loading' : '');?>">
...
<script type="text/javascript">											
	var ppbutton = document.getElementById("btn_play_pause");
	ppbutton.addEventListener("click", playPause);
	myVideo = document.getElementById("video_banner");
	$('.svg-inline-play').css('display', 'none');
	function playPause() { 
	    if (myVideo.paused) {
	        myVideo.play();
	        $('.svg-inline-play').css('display', 'none');
	        $('.svg-inline-pause').css('display', 'inline-block');
	    }else{
	        myVideo.pause(); 
	        $('.svg-inline-pause').css('display', 'none');
	        $('.svg-inline-play').css('display', 'inline-block');
	    }
	}
</script>