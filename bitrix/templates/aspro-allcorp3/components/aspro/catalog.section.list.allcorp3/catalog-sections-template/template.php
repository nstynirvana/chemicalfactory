						...
						<?if($bShowImage && $imageSrc):?>
							<div class="sections-list__item-image-wrapper <?=$imageWrapperClasses?>">
								<a class="sections-list__item-link" href="<?=$detailUrl?>">
									<?if($bIcons && $nImageID):?>
										<?=CAllcorp3::showIconSvg(' fill-theme sections-list__item-image-icon', $imageSrc);?>
									<?else:?>
										<span class="sections-list__item-image <?=$imageClasses?>" style="background-image: url(<?=$imageSrc?>);" title="<?=$arItem['NAME']?>"></span>
									<?endif;?>
								</a>
							...
							<div class="sections-list__item-additional-text-wrapper flex-grow-1">
								<div class="sections-list__item-additional-text-top-part flexbox flexbox--justify-beetwen">
									...
									<?if($arParams['COUNT_ELEMENTS']):?>
										<div class="sections-list__item-products-count font_13 <?=($bFonImg ? 'color_light--opacity' : 'color_999')?>"><?=Aspro\Functions\CAsproAllcorp3::declOfNum(
											$arItem['ELEMENT_CNT'], 
											array(
