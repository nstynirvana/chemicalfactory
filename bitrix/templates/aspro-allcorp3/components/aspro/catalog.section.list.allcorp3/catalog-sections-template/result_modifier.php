...
if (!$arParams['LINKED'] && !($arParams['FRONT_PAGE'] && !$arParams['SHOW_CHILDS'])) {
	...
	if($arChildItems){
		foreach($arChildItems as $key => $arSection){
			$arRootSection = CAllcorp3Cache::CIBlockSection_GetList(array('SORT' => 'ASC','NAME' => 'ASC','CACHE' => array('MULTI' =>'N', 'TAG' => CAllcorp3Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => $arResult['SECTION']['DEPTH_LEVEL'] + 1, 'IBLOCK_ID' => $arParams['IBLOCK_ID']), false, array('ID', 'NAME', 'IBLOCK_ID', 'SORT', 'SECTION_PAGE_URL', 'PICTURE', 'UF_TOP_SEO'));
	...
	{ 
		$arRootSection2ShowOnIndexPage = CAllcorp3Cache::CIBlockSection_GetList(
			array(
				'SORT' => 'ASC',
				'NAME' => 'ASC',
				'CACHE' => array(
