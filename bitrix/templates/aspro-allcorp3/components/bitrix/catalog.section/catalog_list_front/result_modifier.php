...
if (!empty($arResult['ITEMS'])) {?>
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		...
		if (!empty($arItem['DISPLAY_PROPERTIES'])) {
			...
			$arItem['PROPS'] = CAllcorp3::PrepareItemProps($arItem['DISPLAY_PROPERTIES']);
		}
		...
		if ($arItem['IBLOCK_SECTION_ID']) {
			...
			if ($bShowHintTextItem) {
				...
				$arSection = CAllcorp3Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CAllcorp3Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $sectionID, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN", "UF_INCLUDE_TEXT"));
				...
				if (!$sectionHelpText) {
					if ($arSection["DEPTH_LEVEL"] > 2) {
						$arSectionParent = CAllcorp3Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CAllcorp3Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "UF_INCLUDE_TEXT"));
						if (strlen($arSectionParent['UF_INCLUDE_TEXT'])) {
							$sectionHelpText = $arSectionParent['UF_INCLUDE_TEXT'];
						}

						if (!$sectionHelpText) {
							$arSectionRoot = CAllcorp3Cache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CAllcorp3Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $arSection["LEFT_MARGIN"], ">=RIGHT_BORDER" => $arSection["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "UF_INCLUDE_TEXT"));
	...
	if ($arGoodsSectionsIDs) {
		$arGoodsSections = CAllcorp3Cache::CIBLockSection_GetList(array('CACHE' => array('TAG' => CAllcorp3Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N', 'RESULT' => array('NAME'))), array('ID' => $arGoodsSectionsIDs), false, array('ID', 'NAME'));
		foreach ($arResult['ITEMS'] as $key => $arItem) {
			if ($arItem['IBLOCK_SECTION_ID']) {
