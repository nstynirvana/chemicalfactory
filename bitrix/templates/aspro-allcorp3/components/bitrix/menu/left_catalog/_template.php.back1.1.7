		...
		<ul class="menu-side-column__inner">
			...
			foreach( $arResult as $key => $arItem ){
				...
				<li class="menu-side-column__item <?=$bWideMenu ? 'menu-side-column__item--wide' : ''?> fill-theme-parent <?=($counter == 1 ? "menu-side-column__item--first" : "");?> <?=($counter == count($arResult) ? "menu-side-column__item--last" : "");?> <?=($arItem["CHILD"] ? "has-child" : "");?> <?=($arItem["SELECTED"] ? "current opened" : "");?> m_<?=strtolower($arTheme["MENU_POSITION"]["VALUE"]);?> v_<?=strtolower($arTheme["MENU_TYPE_VIEW"]["VALUE"]);?>">
					<a class="menu-side-column__item-link font_15 dark_link <?=($arItem["CHILD"] ? "parent" : "");?>" href="<?=$arItem["LINK"]?>" >
						<?if($arItem["IMAGES"] && $arTheme["LEFT_BLOCK_CATALOG_ICONS"]["VALUE"] == "Y"){?>
							<span class="image">
								<?if(strpos($arItem["IMAGES"]["src"], ".svg") !== false):?>
									<?=CAllcorp3::showIconSvg("cat_icons", $arItem["IMAGES"]["src"]);?>
								<?else:?>
									<img class="lazy" data-src="<?=$arItem["IMAGES"]["src"];?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arItem["IMAGES"]["src"]);?>" alt="<?=$arItem["TEXT"];?>" title="<?=$arItem["TEXT"]?>" /></span>
								<?endif;?>
							</span>
						<?}?>
					...
					<?if($arItem["CHILD"]){?>
						<?=CAllcorp3::showIconSvg(' fill-theme-target header-menu-side__wide-submenu-right-arrow fill-dark-light-block', SITE_TEMPLATE_PATH.'/images/svg/Arrow_right_small.svg');?>
