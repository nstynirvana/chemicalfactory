$(document).ready(function(){
		...
		if(window.matchMedia('(min-width: 992px)').matches)
		{
			try{
				...
				if(menu.length) {
					...
					var childMenu = menu.find('.menu-side-column__dropdown:not(.menu-side-column__dropdown--wide)');
					if(childMenu.length) {
						childMenu.css({
	...
	$('.menu-side-column__item, .menu-side-column__dropdown-item').on('mouseenter', function() {
		var _this = $(this);
		var bWide = _this.hasClass('menu-side-column__item--wide');
		var dropdown = _this.find('> .menu-side-column__dropdown');
		var silder = _this.closest('.menu-side-column').find('.menu-side-column__bottom-banners');
	...
	$(document).on('click', '.header-menu-side__wide-submenu-item-inner .toggle_block', function(e) {
        e.preventDefault();

        var _this = $(this),
        menu = _this.closest(".header-menu-side__wide-submenu-item-inner").find("> .submenu-wrapper");
        ...
        if (!_this.hasClass("clicked")) {
            ...
            _this.closest(".header-menu-side__wide-submenu-item-inner").toggleClass("opened");
        }
    });

	$(document).on('click', '.header-menu-side__wide-submenu-item--more_items', function(e) {
		...
		var useDelimetr = parent.hasClass('header-menu-side__wide-submenu--delimiter');
