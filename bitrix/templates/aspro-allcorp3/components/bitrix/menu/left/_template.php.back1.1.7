			...
			<?foreach($arResult as $arItem):?>
				<li class="<?=($arItem["SELECTED"] ? "active opened" : "")?> <?=($arItem["CHILD"] && !isset($arItem["NO_PARENT"]) ? "child" : "")?>">
					<span class="bg-opacity-theme-parent-hover link-wrapper fill-theme-parent-all">
						...
						<a href="<?=$arItem["LINK"]?>" class="dark_link top-level-link <?=($arItem["SELECTED"] ? " link--active" : "")?> link-with-flag"><?=(isset($arItem["PARAMS"]["BLOCK"]) && $arItem["PARAMS"]["BLOCK"] ? $arItem["PARAMS"]["BLOCK"] : "");?>
