...
if (
	...
	setcookie('catalogLandingViewMode', $_REQUEST['display'], 0, SITE_DIR);
	$_COOKIE['catalogLandingViewMode'] = $_REQUEST['display'];
}
if (array_key_exists('sort', $_REQUEST) && !empty($_REQUEST['sort'])) {
	setcookie('catalogLandingSort', $_REQUEST['sort'], 0, SITE_DIR);
	$_COOKIE['catalogLandingSort'] = $_REQUEST['sort'];
}
if (array_key_exists('order', $_REQUEST) && !empty($_REQUEST['order'])) {
	setcookie('catalogLandingOrder', $_REQUEST['order'], 0, SITE_DIR);
	$_COOKIE['catalogLandingOrder'] = $_REQUEST['order'];
}
...
if (isset($_COOKIE['catalogLandingViewMode']) && $_COOKIE['catalogLandingViewMode']) {
	$display = $_COOKIE['catalogLandingViewMode'];
} else {
	if (
...
$sort = !empty($_COOKIE['catalogLandingSort']) ? $_COOKIE['catalogLandingSort'] : $sort_default;
$order = !empty($_COOKIE['catalogLandingOrder']) ? $_COOKIE['catalogLandingOrder'] : $order_default;
	...
	<div class="filter-panel__part-left">
		<div class="line-block line-block--align-normal line-block--0 filter-panel__main-info">
			<?if($arTheme['SHOW_SMARTFILTER']['VALUE'] !== 'N' && $itemsCnt):?>
				<?$bActiveFilter = \Aspro\Functions\CAsproAllcorp3::checkActiveFilterPage($arParams["SEF_URL_TEMPLATES"]['smart_filter']);?>
				<div class="line-block__item filter-panel__filter <?=($bHideLeftBlock && !$bShowCompactHideLeft ? 'filter-panel__filter--visible' : '');?>">
					<div class="fill-theme-hover dark_link">
						<div class="bx-filter-title filter_title <?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? 'active-filter' : '')?>">
							<?=CAllcorp3::showIconSvg("icon", SITE_TEMPLATE_PATH.'/images/svg/catalog/filter.svg', '', '', true, false);?>
				...
				<div class="line-block__item">
					<div class="filter-panel__sort">
						<div class="dropdown-select dropdown-select--wide">
							<div class="dropdown-select__title font_14">
								<span>
									<?if($order && $sort):?>
								...
								<?=CAllcorp3::showIconSvg("down", SITE_TEMPLATE_PATH.'/images/svg/Triangle_down.svg', '', '', true, false);?>
							</div>
					...
					<div class="line-block__item">
						<div class="filter-panel__view controls-linecount controls-view">
							...
							<?if (
								...
								array_key_exists("linerowLanding", $_SESSION) || 
								...
								if (
									...
									$_SESSION["linerowLanding"]=htmlspecialcharsbx(trim($_REQUEST["linerow"]));
								} elseif (
									$_SESSION["linerowLanding"] && 
									(in_array(trim($_SESSION["linerowLanding"]), $arLineCount))
								) {
									$linerow = $_SESSION["linerowLanding"];
								} elseif(
									$arParams["SECTION_LIST_DISPLAY_TYPE"] && 
							...
							<?foreach ($arLineCount as $value):?>
								...
								?>
								<?if($linerow == $value):?>
									<span title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".$value)?>" class="controls-view__link controls-view__link--current"><?=CAllcorp3::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$value.'inarow.svg', '', '', true, false);?></span>
								<?else:?>
									<a rel="nofollow" href="<?=$url;?>" data-url="<?=$url?>" title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".$value)?>" class="controls-view__link muted fill-theme-hover <?=($arParams['AJAX_CONTROLS'] == 'Y' ? ' js-load-link' : '');?>"><?=CAllcorp3::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$value.'inarow.svg', '', '', true, false);?></a>
								<?endif;?>
							<?endforeach;?>
							<div class="controls-hr"></div>
						</div>
					...
					<div class="filter-panel__view controls-view">
						<?foreach($arDisplays as $displayType):?>
							...
							<?if($display == $displayType):?>
								<span title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".strtoupper($displayType))?>" class="controls-view__link controls-view__link--<?=$displayType?> controls-view__link--current"><?=CAllcorp3::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$displayType.'type.svg', '', '', true, false);?></span>
							<?else:?>
								<a rel="nofollow" href="<?=$url;?>" data-url="<?=$url?>" title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".strtoupper($displayType))?>" class="controls-view__link controls-view__link--<?=$displayType?> muted fill-theme-hover <?=($arParams['AJAX_CONTROLS'] == 'Y' ? ' js-load-link' : '');?>"><?=CAllcorp3::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$displayType.'type.svg', '', '', true, false);?></a>
							<?endif;?>
						<?endforeach;?>
					</div>
				</div>
