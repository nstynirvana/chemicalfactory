								...
								<div class="complects-list__item-text-wrapper flexbox" data-id="<?= $arItem['ID'] ?>" <?= ($bOrderViewBasket ? ' data-item="' . $dataItem . '"' : '') ?>>
									<?if ($arItem['LINK_COMPLECT'] || $arItem['LINK_SERVICES']):?>
										...
										<div class="complects-list__item-text-top-part text-complect" data-currency="<?=$currency;?>">
											<? if ($arItem['LINK_COMPLECT']):?>
												<?
												$totalPrice = $arItem['DISPLAY_PROPERTIES']['FILTER_PRICE']['VALUE'] ?? CAsproAllcorp3::clearPriceFromString($arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE']);
												$totalPriceOld = CAsproAllcorp3::clearPriceFromString($arItem['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE']);
												$totalPriceDiff = CAsproAllcorp3::clearPriceFromString($arItem['DISPLAY_PROPERTIES']['ECONOMY']['VALUE']);
												?>
												<div class="complects-list__item-properties-wraper">
													...
													<? foreach ($arItem['LINK_COMPLECT'] as $arProperty) : ?>
														<?
														$linkComplectPrice = $arProperty['FILTER_PRICE']['VALUE'] ?? CAsproAllcorp3::clearPriceFromString($arProperty['PRICE']['VALUE']);
														$linkComplectPriceOld = CAsproAllcorp3::clearPriceFromString($arProperty['PRICEOLD']['VALUE']);
															...
															<div class="line-block line-block--align-normal line-block--20 line-block--20-vertical">
																...
																<div class="line-block__item complect-props__value">
																	<span class="complect-props__inner font_13 text-right">
																		<?= CAsproAllcorp3::showPrice(['ITEM' => $arProperty, 'PARAMS' => $arParams, 'PRICE_FONT' => 13, 'SHOW_SCHEMA' => false]) ?>
																	</span>
																</div>
															</div>
														</div>
												...
												<?
												if ($totalPrice) {
													$arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE'] = CAllcorp3::FormatSumm($totalPrice).' '.$currency;
												}
												if ($totalPriceOld) {
													$arItem['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE'] = CAllcorp3::FormatSumm($totalPriceOld).' '.$currency;
												}
												if ($totalPriceDiff) {
													$arItem['DISPLAY_PROPERTIES']['ECONOMY']['VALUE'] = CAllcorp3::FormatSumm($totalPriceDiff).' '.$currency;
												}?>
											<?endif;?>
													...
													<? foreach ($arItem['LINK_SERVICES'] as $arService) : ?>
														<div class="complects-list__item-properties color_333 complect-props">
															<div class="line-block line-block--align-normal line-block--20 line-block--10-vertical">
																...
																$price = ($arService['PROPERTY_FILTER_PRICE_VALUE'] ?? CAsproAllcorp3::clearPriceFromString($arService['PROPERTY_PRICE_VALUE']));
																$priceOld = (CAsproAllcorp3::clearPriceFromString ($arService['PROPERTY_PRICEOLD_VALUE']??$arService['PROPERTY_PRICE_VALUE']));
																?>
																<div class="line-block__item complect-props__name"  <?= ($bOrderViewBasket ? ' data-item="' . CAllcorp3::getDataItem($arService) . '"' : '') ?> data-id="<?=$arService['ID'];?>">
																...
																<div class="line-block__item complect-props__value">
																	<span class="font_13 text-right">
																		<?= CAsproAllcorp3::showPrice(['ITEM' => $arService, 'PARAMS' => $arParams, 'PRICE_FONT' => 13, 'SHOW_SCHEMA' => false]) ?>
																	</span>
																</div>
															</div>
														</div>
									...
									<? if ($bShowBottom) : ?>
										<div class="complects-list__item-text-bottom-part">
											<?CAsproAllcorp3::showPrice(['ITEM' => $arItem, 'PARAMS' => $arParams, 'SHOW_SCHEMA' => false, 'PRICE_FONT' => '22']); ?>
											<? if ($bOrderButton) : ?>
