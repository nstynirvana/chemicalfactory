...
if($bItemsTypeAlbums){
	...
	foreach($arResult['ITEMS'] as $key => $arItem){
		if($arPhotos = (array_key_exists('VALUE', $arItem['PROPERTIES']['PHOTOS']) && $arItem['PROPERTIES']['PHOTOS']['VALUE']) ? (array)$arItem['PROPERTIES']['PHOTOS']['VALUE'] : (array)$arItem['PROPERTY_PHOTOS_VALUE']){
			foreach($arPhotos as $photoId){
				if(in_array($photoId, $arNewPhotos)){
					...
					CAllcorp3::getFieldImageData($arNewItem, array('PREVIEW_PICTURE'));
