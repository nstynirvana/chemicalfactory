							...
							<?foreach($arResult['ITEMS'] as $arItem):?>
								...
								<div class="item-accordion-wrapper shadow-hovered shadow-no-border-hovered" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
									<a class="accordion-head accordion-close stroke-theme-hover" data-toggle="collapse" data-parent="#accordion<?=$arItem['ID']?>" href="#accordion<?=$arItem['ID']?>">
										...
										<?=CAllcorp3::showIconSvg('right-arrow', SITE_TEMPLATE_PATH.'/images/svg/Plus_lg.svg');?>
									</a>
												...
												<?if ($bShowBtn):?>
													<div class="accordion-btn">
														<?if ($arItem['DISPLAY_PROPERTIES']['FORM_CODE']['VALUE']):?>
															<span class="btn btn-default btn-transparent-border animate-load" data-event="jqm" data-param-id="<?=CAllcorp3::getFormID($arItem['DISPLAY_PROPERTIES']['FORM_CODE']['VALUE'])?>" data-name="question"><?=$arItem['DISPLAY_PROPERTIES']['TITLE_BUTTON']['VALUE'];?></span>
														<?else:?>
