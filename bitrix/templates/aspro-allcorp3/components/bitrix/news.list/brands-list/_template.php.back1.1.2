...
<?if($arResult['ITEMS']):?>
	...
	$bTopNav = (!$arParams['TITLE_CENTER'] && $arParams['SHOW_TITLE']) && $arParams['NARROW'] && $arParams['SLIDER'];
	...
	$owlClasses = ' owl-carousel--light owl-carousel--buttons-bordered';
	$owlClasses .= ' owl-carousel--items-width-230-adaptive';
	...
	<?endif;?>
				<?foreach($arResult['ITEMS'] as $itemKey => $arItem):?>
					...
					?>
					<div class="grid-list__item <?=($arParams['SLIDER'] ? 'hover_blink' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
