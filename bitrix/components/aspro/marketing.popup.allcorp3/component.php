...
if(!$arResult['IS_AJAX']){
	$ob = new Aspro\Allcorp3\MarketingPopup();
	$rules = $ob->getRules();
	...
	if($request['id']){
		...
		if($arItem = $dbRes->Fetch()){
			...
			$arResult['ITEM'] = CAllcorp3Cache::CIBLockElement_GetList(array(
				'CACHE' => array(
					'TAG' => CAllcorp3Cache::GetIBlockCacheTag($arItem['IBLOCK_ID']),
					'MULTI' => 'N'
				)),
				$arFilter,
				false,
			...
			if($arResult['ITEM']){
				...
				if($type === 'WEBFORM'){
					$arResult['ITEM']['PROPERTY_LINK_WEB_FORM_ID'] = intval(CAllcorp3::getFormID($arResult['ITEM']['PROPERTY_LINK_WEB_FORM_VALUE']));
				}
