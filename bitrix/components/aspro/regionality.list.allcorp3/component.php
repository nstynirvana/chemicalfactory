...
<?if(\Bitrix\Main\Loader::includeModule('aspro.allcorp3'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.allcorp3', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
		$arResult['REGIONS'] = CAllcorp3Regionality::getRegions();
		$arResult['CURRENT_REGION'] = CAllcorp3Regionality::getCurrentRegion();
		$arResult['REAL_REGION'] = CAllcorp3Regionality::getRealRegionByIP();
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CAllcorp3Cache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CAllcorp3Cache::GetIBlockCacheTag(CAllcorp3Cache::$arIBlocks[SITE_ID]['aspro_allcorp3_regionality']['aspro_allcorp3_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CAllcorp3Cache::$arIBlocks[SITE_ID]['aspro_allcorp3_regionality']['aspro_allcorp3_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CAllcorp3Cache::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CAllcorp3Cache::GetIBlockCacheTag(CAllcorp3Cache::$arIBlocks[SITE_ID]['aspro_allcorp3_regionality']['aspro_allcorp3_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CAllcorp3Cache::$arIBlocks[SITE_ID]['aspro_allcorp3_regionality']['aspro_allcorp3_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.allcorp3', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
