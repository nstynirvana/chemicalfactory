...
$arParams["SHOW_PROPS"] = CAllcorp3::GetFrontParametrValue('SHOW_PROPS_BLOCK');
$arParams["SHOW_ONE_CLICK_BUY"] = CAllcorp3::GetFrontParametrValue('SHOW_ONE_CLICK_BUY');
$arParams["SHOW_GALLERY"] = CAllcorp3::GetFrontParametrValue('SHOW_CATALOG_GALLERY_IN_LIST');
$arParams["MAX_GALLERY_ITEMS"] = CAllcorp3::GetFrontParametrValue('MAX_GALLERY_ITEMS');
$arParams["ADD_PICT_PROP"] = CAllcorp3::GetFrontParametrValue('GALLERY_PROPERTY_CODE');
$arParams["USE_FAST_VIEW_PAGE_DETAIL"] = CAllcorp3::GetFrontParametrValue('USE_FAST_VIEW_PAGE_DETAIL');
$arParams["DISPLAY_COMPARE"] = CAllcorp3::GetFrontParametrValue('CATALOG_COMPARE');
...
$arParams["CHECK_REQUEST_BLOCK"] = CAllcorp3::checkRequestBlock('catalog_tab');
$arParams["AJAX_REQUEST"] = CAllcorp3::checkAjaxRequest() && $arParams["CHECK_REQUEST_BLOCK"] ? 'Y' : 'N';
...
if ($bCatalogIndex && $arParams["HIT_PROP"]) {
	...
	if ($arShowProp) {
		foreach ($arShowProp as $key => $prop) {
			...
			$arItems = CAllcorp3Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI" => "N", "TAG" => CAllcorp3Cache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array_merge($arFilter, $arrFilter, $arFilterProp), false, array("nTopCount" => 1), array("ID"));
			if ($arItems) {
				$arTabs[$key] = array(
					"CODE" => $key,
					"TITLE" => $prop,
