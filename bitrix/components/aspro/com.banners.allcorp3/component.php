...
$APPLICATION->SetAdditionalCss("/bitrix/components/aspro/com.banners.allcorp3/styles.css");

$arParams['CURRENT_BANNER_INDEX'] = Solution::getCurrentPresetBannerIndex(SITE_ID);
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		foreach($arBannersCode as $key => $arTypeBaner)
		{
			...
			while($obElement = $rsElement->GetNextElement())
			{
				...
				$arItem['TIZERS'] = Cache::CIblockElement_GetList(array("SORT" => 'ASC', 'ID' => 'ASC', "CACHE" => array("TAG" => Cache::GetIBlockCacheTag($arParams["TIZERS_IBLOCK_ID"]), 'GROUP' => array('ID'))), $arrTizersFilter, false, false, $arrTizersSelect);
