...
<?$arThemes = CAllcorp3::GetFrontParametrsValues(SITE_ID);
...
if(\Bitrix\Main\Loader::includeModule('aspro.allcorp3'))
{
	...
	if($arThemes['USE_BITRIX_FORM'] == 'Y' && \Bitrix\Main\Loader::includeModule('form')):?>
		...
		// if (isset($arParams['WEBFORM_ID'])) {
			...
			if ($item = $rsForm->Fetch()) {
				...
				$bCRMForm = \Bitrix\Main\Config\Option::get('aspro.allcorp3', strtoupper($code).'_FORM', 'COMPLEX', SITE_ID) == 'CRM';
				$formCode = $code;
				$formName = $item['NAME'];
			}
		// }
	...
	<?else:?>
		<?if( CModule::IncludeModule("iblock") ){
			...
			$arImgID = unserialize(\Bitrix\Main\Config\Option::get('aspro.allcorp3', 'BGFILE'.strtoupper($arParams["IBLOCK_ID"]).'_FORM'.$imgType, serialize([]), SITE_ID));
			...
			if (!$bFormID) {
				...
				$arParams["IBLOCK_ID"] = CAllcorp3Cache::$arIBlocks[SITE_ID]["aspro_allcorp3_form"][$arParams["IBLOCK_ID"]][0];
			...
			if (!$bCRMForm) {
				...
				if( !is_array( $arMess ) ){
					...
					$arMess["ID"] = $em->Add( array(
						...
						"EMAIL_TO" => ($arResult["IBLOCK_CODE"] == "aspro_allcorp3_callstaff" ? "#STAFF_EMAIL_HIDDEN#" : "#EMAIL#"),
						"BCC" => "",
					...
					foreach( $arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion ){
						...
						if ($FIELD_CODE == 'RATING') {?>
							...
							<div class="votes_block with-text">
								<div class="ratings">
									<?for($i=1;$i<=5;$i++):?>
										<div class="item-rating" data-message="<?=GetMessage('RATING_MESSAGE_'.$i)?>"><?=\CAllcorp3::showIconSvg("star", SITE_TEMPLATE_PATH."/images/svg/star.svg");?></div>
									<?endfor;?>
								</div>
						...
						if( count( (array)$arResult["FORM_ERRORS"] ) <= 0 ){
								...
								if( $RESULT_ID = $el->Add( $arFields ) ){
									...
									if(strlen($arEventFields["EMAIL"])){
										if(isset($arEventFields['ORDER_LIST']) && strpos('aspro_allcorp3_order_page', $arResult['IBLOCK_CODE']) !== false){
