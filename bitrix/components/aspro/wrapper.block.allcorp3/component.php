...
$bError = \Bitrix\Main\Loader::includeModule('aspro.allcorp3') == false;
...
$arParams["IS_AJAX"] = CAllcorp3::checkAjaxRequest();
//$arParams["USE_FAST_VIEW"] = CAllcorp3::GetFrontParametrValue('USE_FAST_VIEW_PAGE_DETAIL');

//$arParams['DISPLAY_WISH_BUTTONS'] = CAllcorp3::GetFrontParametrValue('CATALOG_DELAY');
/**/
	...
	if ($arParams["IS_AJAX"] != 'Y') {
		...
		if ($arRegion) {
			...
			if ($arParams['FILTER_NAME'] == 'arRegionality' && $arParams['STORES']) {
				if (CAllcorp3::GetFrontParametrValue('STORES_SOURCE') != 'IBLOCK') {
			...
			if ($arParams['FILTER_NAME'] == 'arRegionLink' && CAllcorp3::GetFrontParametrValue('REGIONALITY_FILTER_ITEM') == 'Y') {
